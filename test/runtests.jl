...
@testset "KidneyExchange.jl" begin
	tests_K3_L0 = Dict(joinpath("tests_cycles","MD-00001-00000001") => 4, 
    ...
    for filename in keys(tests_K3_L0)
        ...
        stats = KidneyExchange.solve_with_mip(filename, 3, 0, MIP_params(KidneyExchange.HPIEF));
        println("\t", @test stats[1].objective_value == tests_K3_L0[filename]);

        stats = KidneyExchange.solve_with_mip(filename, 3, 0, MIP_params(KidneyExchange.EXTENDED_EDGE));
        println("\t", @test stats[1].objective_value == tests_K3_L0[filename]);

        stats = KidneyExchange.solve_with_mip(filename, 3, 0, MIP_params(KidneyExchange.RELAXED_ARC));
        println("\t", @test stats[1].objective_value >= tests_K3_L0[filename]);
    end

	tests_K4_L0 = Dict(joinpath("tests_cycles","MD-00001-00000001") => 4, 
    ...
    for filename in keys(tests_K4_L0)
        ...
        # test compact formulations
        stats = KidneyExchange.solve_with_mip(filename, 4, 0, MIP_params(KidneyExchange.HPIEF));
        println("\t", @test stats[1].objective_value == tests_K4_L0[filename]);

        stats = KidneyExchange.solve_with_mip(filename, 4, 0, MIP_params(KidneyExchange.EXTENDED_EDGE));
        println("\t", @test stats[1].objective_value == tests_K4_L0[filename]);

        stats = KidneyExchange.solve_with_mip(filename, 4, 0, MIP_params(KidneyExchange.RELAXED_ARC));
        println("\t", @test stats[1].objective_value >= tests_K4_L0[filename]);
    end

	tests_K3_L6 = Dict(joinpath("tests_chains","MD-00001-00000015") => 16, 
    ...
    for filename in keys(tests_K3_L6)
        ...
        # test compact formulations
        stats = KidneyExchange.solve_with_mip(filename, 3, 6, MIP_params(KidneyExchange.HPIEF));
        println("\t", @test stats[1].objective_value == tests_K3_L6[filename]);

        stats = KidneyExchange.solve_with_mip(filename, 3, 6, MIP_params(KidneyExchange.EXTENDED_EDGE));
        println("\t", @test stats[1].objective_value == tests_K3_L6[filename]);
    end
end
