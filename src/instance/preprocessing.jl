...
To get the source vertices of donor/patient pairs, compute a feedback vertex set (FVS) that heuristically minimizes the cardinality of the set.  Each vertex of the FVS will be the source of one copy in the column generation subproblems. Then compute distances from and to the source in each subgraph. These will be used in the Bellman-Ford search at each solution of the subproblems.


# Input parameters
* `instance::Instance` : The structure describing the characteristics of an instance including the compatibility graph
* `reduce_arcs::Bool` : A boolean parameter indicating whether or not arcs that are not useful should be eliminated from the instance (default value is false)

# Output parameters
* `subgraphs::SubgraphsData` : The structure describing the preprocessed data of the graph copies
"""
function preprocess_graph_copies(instance::Instance, reduce_arcs::Bool=false, reduce_vertices::Bool = false)
    ...
    while true
        ...
        # delete the source vertex in the remaining copies
        is_deleted[vstar] = true
