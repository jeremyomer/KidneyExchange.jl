...
function node_master(
    ...
    master =
        create_model(time_limit, bp_params.optimizer, false, false, bp_params.nb_threads)
    ...
    if !bp_params.is_pief || L == 0
        @constraint(
            master,
            capacity[v in vertices(graph)],
            sum(y[c] for c = 1:length(column_pool) if v in (column_pool[c]).vertices) <= 1
        )
    else
        @constraint(
            master,
            capacity[v in vertices(graph)],
            sum(y[c] for c = 1:length(column_pool) if v in (column_pool[c]).vertices) +
            sum(
                chain_flow[u, v, k] for u in inneighbors(graph, v), k = l_min[u]:l_max[u]
            ) <= 1
        )
    end
    ...
    if bp_params.is_pief && L >= 1
        ...
        if L >= 2
            @constraint(
                master,
                [u in instance.pairs],
                sum(chain_flow[v, u, 1] for v in altruist_neighbors[u]) -
                sum(chain_flow[u, v, 2] for v in outneighbors(graph, u)) >= 0
            )
        end
        @constraint(
            master,
            [u in instance.pairs, k in 2:L-1],
            sum(chain_flow[v, u, k] for v in pair_neighbors[u]) -
            sum(chain_flow[u, v, k+1] for v in outneighbors(graph, u)) >= 0
        )
        # altruists can start no more than one chain
        @constraint(
            master,
            [u in instance.altruists],
            sum(chain_flow[u, v, 1] for v in outneighbors(graph, u)) <= 1
        )
    end
    ...
    @constraint(
        master,
        branch_nb_arcs_max,
        sum(y[c] for c = 1:length(column_pool)) <= nv(graph)
    )
    @constraint(
        master,
        branch_nb_arcs_min,
        sum(y[c] for c = 1:length(column_pool)) + slack >= 0
    )
    ...
    if bp_params.is_pief && L >= 1
        @objective(
            master,
            Max,
            sum(column_pool[c].weight * y[c] for c = 1:length(column_pool)) +
            sum(
                instance.edge_weight[u, v] * chain_flow[u, v, 1] for u in
                                                                     instance.altruists for
                v in outneighbors(graph, u)
            ) +
            sum(
                instance.edge_weight[u, v] * chain_flow[u, v, k] for u in instance.pairs for
                v in outneighbors(graph, u) for k = 2:L
            ) - nv(graph) * max_cost * slack
        )
    else
        @objective(
            master,
...
function initialize_master_IP(
    ...
    if bp_params.is_pief && L >= 1
        ...
        @objective(
            master,
            Max,
            (1.0 + 1 / (nb_pairs * max_cost)) *
            sum(column_pool[c].weight * y[c] for c = 1:length(column_pool)) +
            sum(
                W[u, v] * chain_flow[u, v, 1] for u in instance.altruists for
                v in outneighbors(graph, u)
            ) +
            sum(
                W[u, v] * chain_flow[u, v, k] for u in instance.pairs for
                v in outneighbors(graph, u) for k = 2:L
            )
        )
    else
