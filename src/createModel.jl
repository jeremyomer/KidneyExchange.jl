function create_model(time_limit::Float64, optimizer::String, is_integer::Bool = false, verbose::Bool = false)
  if optimizer=="CPLEX"
    ...
    set_optimizer_attribute(model, "CPX_PARAM_SCRIND", Int(verbose))
    set_optimizer_attribute(model, "CPX_PARAM_MIPDISPLAY", Int(verbose))
    set_optimizer_attribute(model, "CPX_PARAM_TILIM", max(0,time_limit))
  elseif optimizer=="Gurobi"
    model = Model(Gurobi.Optimizer)
    set_optimizer_attribute(model, "OutputFlag", Int(verbose))
    set_optimizer_attribute(model, "TimeLimit", max(0,time_limit))
