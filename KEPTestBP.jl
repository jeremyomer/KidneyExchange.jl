...
println(
    bp_log,
    "$filename; $(graph_info.nb_pairs); $(graph_info.nb_altruists); $cycle_limit; $chain_limit; BP; $(bp_status.objective_value); $(round(t_total/10^8)/10); $(100*bp_status.relative_gap); $(bp_status.bp_info.nb_col_root); $(bp_status.node_count); $(round(10*t_node_master)/10) %; $n_call_node_master; $(round(10*t_bellman)/10) %; $ncall_bellman; $(round(10*t_mip_master)/10) %; $ncall_mip_master; $(round(10*t_parser)/10) %; $(round(10*t_preprocess)/10) %",
)
